...
export default function NewDesignationPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            router.push(`/settings/designations/${response.id}`);
        } catch (error) {
    ...
    return (
        <div className="space-y-6 animate-fade-in-up">
            {/* Header */}
            <div>
                <Link href="/settings/designations" className="inline-flex items-center gap-1 text-gray-500 hover:text-gray-700 mb-2">
                    <HiOutlineArrowLeft className="w-4 h-4" /> Back to Designations
                </Link>
                <h1 className="text-3xl font-bold tracking-tight text-foreground">Create New Designation</h1>
                <p className="text-muted-foreground mt-1">Add a new job title or designation</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end gap-3">
                    ...
                    <Button variant="primary" type="submit" loading={saving}>
                        <HiOutlineCheck className="w-4 h-4" /> Create Designation
                    </Button>
                </div>
            </form>
        </div>
